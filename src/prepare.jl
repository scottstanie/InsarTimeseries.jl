...
function save_masks(
    ...
    geo_mask_stack = load_stack(directory = igram_path, file_ext = ".geo.mask")
    ...
    int_mask_stack =
        Array{eltype(geo_mask_stack),3}(undef, (nrows, ncols, length(int_file_list)))
    ...
    save_hdf5_stack(
        mask_filename,
        igram_dset_name,
        convert(Array{Bool}, int_mask_stack),
        overwrite = false,
    )
    save_hdf5_stack(
        mask_filename,
        geo_dset_name,
        convert(Array{Bool}, geo_mask_stack),
        overwrite = false,
    )
...
function _remaining_files(
    in_files::Array{String,1},
    out_files::Array{String,1},
    overwrite::Bool,
)
    if overwrite
...
sliceview(stack) = [view(stack, :, :, i) for i = 1:size(stack, 3)]
...
function create_hdf5_stack(
    ...
    get_file_ext(filename) in (".h5", ".hdf5") ||
    ArgumentError("filename must end in .h5 or .hdf5")
...
function shift_unw_file(
    unw_stack_file::String;
    stack_flat_dset = nothing,
    ref_row = nothing,
    ref_col = nothing,
    window = 5,
    ref_station = nothing,
    ...
    if (isnothing(ref_row) || isnothing(ref_col))
        isnothing(ref_station) &&
        throw(ArgumentError("Need ref_station if no ref_row/ref_col"))
    ...
    h5writeattr(
        unw_stack_file,
        stack_flat_shifted_dset,
        Dict(REFERENCE_ATTR => [ref_row, ref_col]),
    )
    ...
    h5writeattr(
        unw_stack_file,
        stack_flat_shifted_dset,
        Dict(REFERENCE_STATION_ATTR => station_str),
    )
...
function shift_stack(stack_in, stack_out, ref_row::Int, ref_col::Int; window::Int = 5)
...
function _create_dset(h5file, dset_name, shape, dtype)
    ...
    h5open(h5file, "r+") do f
        d_create(f, dset_name, datatype(dtype), dataspace(shape))
    end
end
