...
function demo_point(rowcol; sigma=3, max_temp=800, show=true, refpoint=nothing, max_date=nothing,
                    unwfile="unw_stack.h5", input_dset= "stack_flat_shifted", ignore_file="geolist_ignore.txt")
    geolist, intlist, igram_idxs = load_geolist_intlist(unwfile, ignore_file, max_temp, max_date=max_date)
    B = InsarTimeseries.build_B_matrix(geolist, intlist);
    Blin = sum(B, dims=2);
    geolist, intlistall, igram_idxs_all = load_geolist_intlist(unwfile, ignore_file, 2000, max_date=max_date)
