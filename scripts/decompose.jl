...
function solve_east_up(
    asc_path::AbstractString,
    desc_path::AbstractString,
    asc_fname::AbstractString,
    desc_fname::AbstractString = asc_fname,
    dset = "velos/1",
)
...
function save_east_up_decomp(
    ...
    dd = Dict("lats" => gy, "lons" => gx, "up" => upcut.image, "east" => eastcut.image)
...
eeups(fname, shifta, shiftd, dset = "velos/1", full = true; show = true) =
    extrema.(eastup_insar(
        station_overlap,
        demo_east_up(
            fname;
            full = full,
...
eeups_diff(fname, shifta, shiftd, dset = "velos/1", full = true; show = true) =
    eastup_diffs(
        station_overlap,
        demo_east_up(
            fname;
